...
To configure Rugix Ctrl for such custom use cases, please have a look at the [System Configuration](./advanced/system-configuration) section of this documentation.
...
If you want your own integration, check out the [documentation on boot flows](./advanced/boot-flows).
:::
...
[^non-a-b]: The following commands assume that you use a traditional A/B update scheme. If you use a non-A/B update scheme, the commands will require you to explicitly specify the [boot group](./advanced/system-configuration).
...
See [Signed Updates](./signed-updates) for details on signing bundles and configuring signature verification.
