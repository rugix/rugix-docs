...
Migrating from Mender to Rugix Ctrl requires a custom [system configuration file](../../updates/system-updates/system-configuration) which specifies the correct Mender-compatible boot flow, boot groups, and slots.
...
Any custom integrations such as state scripts need to be adapted to work with Rugix Ctrl, e.g., by using [hooks](../../reference/hooks).

Furthermore, if you want to use Rugix's state management functionality, there needs to be a migration of the data partition to the structure required by Rugix. In this case, you also need to set `boot-dir` to `/run/rugix/mounts/config`, as this is where Rugix's state management functionality will mount the config partition (first partition) with the GRUB environment files to.
