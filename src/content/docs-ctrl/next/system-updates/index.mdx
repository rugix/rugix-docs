...
import { SchemaViewer } from "@silitics/astro-schema-viewer";
import OutputSchema from "../../../../schemas/rugix-ctrl-output-info.schema.json";

# System Updates

Rugix Ctrl's core functionality is the safe installation of _over-the-air_ (OTA) **system updates** — atomic replacements of an entire root filesystem (and optionally the boot partition) with rollback if anything goes wrong.

For lifecycle-managed application workloads (Docker Compose stacks, systemd services, custom scripts), see [Application Updates](../application-updates/) instead.

## A/B Update Scheme

A typical Rugix Ctrl setup uses an _A/B update scheme_ with redundant system partitions (A and B).
...
This drastically reduces the likelihood of bricking devices in the field due to corrupted or incompatible software and failed or incomplete updates.
In addition, A/B updates have the following advantages:
...
See [System Configuration](./system-configuration) for the slot and boot-group model that supports these layouts.

## Bootloader Integration

To commit and switch between systems, Rugix Ctrl needs to integrate with the bootloader of the system.
Rugix Ctrl ships ready-made [boot flows](./boot-flows) for U-Boot, GRUB, `systemd-boot`, and Raspberry Pi's `tryboot` mechanism, plus RAUC- and Mender-compatible flows for [in-field migration](../migrating/).
...
If you want your own integration, see [Boot Flows](./boot-flows).
:::

## Typical Partition Layout

A typical partition layout of a Rugix system following the A/B update scheme comprises six partitions:
...
You will find the most recent version of this schema [on GitHub](https://github.com/rugix/rugix/blob/main/schemas/rugix-ctrl-output-info.schema.json).

## Installing an Update

Updates are delivered to devices in the form of [update bundles](../update-bundles).
Update bundles are installed with the following command:[^non-a-b]

[^non-a-b]: The following commands assume that you use a traditional A/B update scheme. If you use a non-A/B update scheme, the commands will require you to explicitly specify the [boot group](./system-configuration).
...
Streaming updates are not only faster, since the updates have to be written to storage only once, but also have the advantage that they do not waste write cycles and do not take up precious space on the data partition during the installation.
...
You can also _defer_ the reboot with `--reboot deferred` (only for A/B setups, requires [State Management](../state-management/)).
...
When installing an update, Rugix Ctrl requires either a valid [embedded signature](../signed-updates) or a bundle hash (via `--bundle-hash`).
If neither can be established, the installation will be refused.

To verify the integrity of an update bundle using a hash, use the `--bundle-hash <hash>` option.
This option takes a hash of the bundle header, which is then used to ensure the integrity of the bundle, preventing any manipulated data from ever being written.

For signature-based verification, Rugix Ctrl supports [embedded CMS signatures](../signed-updates) in update bundles.
Root certificates for verification can be configured in `/etc/rugix/ctrl.toml` or provided via the `--root-cert` CLI option.
See [Signed Updates](../signed-updates) and the [reference](../reference/signed-updates) for full details.
...
:::tip
Use a [_pre-commit hook_](../hooks#system-update-hooks) to ensure that a commit is only possible when the system is in proper working order.
:::
