...
To this end, a boot flow needs to interface with a bootloader.
Rugix Ctrl provides the following _default boot flows_:

- `uboot`: Boot flow for U-Boot (A/B updates without a dead man's switch).
- `grub`: Boot flow for GRUB (A/B updates without a dead man's switch).
- `systemd-boot`: Boot flow for `systemd-boot` (A/B updates without a dead man's switch).

For compatibility with other OTA update solutions, Rugix Ctrl further provides the following boot flows:

- `rauc-uboot`: RAUC-compatible boot flow for U-Boot (priority-based).
- `rauc-grub`: RAUC-compatible boot flow for GRUB (priority-based).
- `mender-uboot`: Mender-compatible boot flow for U-Boot (A/B updates).
- `mender-grub`: Mender-compatible boot flow for GRUB (A/B updates).

:::note
Those boot flows allow [safe, in-the-field migrations to Rugix Ctrl from RAUC and Mender](../../integration/migrating/), respectively.
...
Furthermore, it allows existing bootloader and board integrations for RAUC and Mender to be re-used with Rugix Ctrl, even in new projects.
:::
...
Boot flows are configured through the [`boot-flow` section of the system configuration](./system-configuration).
...
As this is complex and difficult to implement correctly, Rugix Ctrl's default boot flows do not include a dead man's switch.
...
See the [RAUC migration guide for details](../../integration/migrating/from-rauc).

## Mender-compatible Boot Flows

The Mender-compatible boot flows interact with the bootloader in the same way as Mender.

See the [Mender migration guide for details](../../integration/migrating/from-mender).
...
For further details, see the reference [boot scripts](https://github.com/rugix/rugix/tree/main/boot/u-boot/scripts) for Raspberry Pi.
...
Still, Rugix Ctrl does its best by first creating a new file and then replacing the old one with the new one by renaming it, and the Linux kernel does guarantee atomicity for renaming.
