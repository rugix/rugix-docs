---
title: Update Bundles Reference
order: 10
---

import { SchemaViewer } from "@silitics/astro-schema-viewer";
import ConfigSchema from "../../../../schemas/rugix-bundle-manifest.schema.json";

# Update Bundles Reference

This page documents the bundle format in detail: payload model, the `rugix-bundler` CLI, block encoding, deduplication, and the full manifest schema. For an introduction, start with [Update Bundles](../update-bundles).

## Payload Model

An update bundle is a single file that may contain multiple update _payloads_.
Typically, for each payload, the update bundle specifies a slot (see [System Configuration: Slots and Boot Groups](../system-updates/system-configuration#update-slots)) to which the payload should be installed.

A few terms to keep apart:

- _Slot Data_: Data that will exist in a slot after a successful update.
- _Payload File_: Single file corresponding to a payload in an update bundle.
- _Payload Data_: Actual data of a payload stored in an update bundle.

For instance, if a slot corresponds to a block device, then the slot data _and_ payload file may be a filesystem image.
However, if a slot corresponds to a directory, then the slot data is the content of this directory and the payload file may be a Tar archive of the directory's contents.
The payload data, in turn, is derived from the payload file — for instance, the payload file may be compressed before being stored in an update bundle.
While the payload file must be reconstructible from the update bundle alone, the slot data may or may not be reconstructible from the bundle alone.
...
For full system updates, a boot group must be provided to `rugix-ctrl` as an installation target (if there are more than two boot groups).
...
You can download pre-built binaries from [the Releases page of Rugix's Git repository](https://github.com/rugix/rugix/releases/).
...
A bundle directory has the following structure:
...
```toml
#:schema https://raw.githubusercontent.com/rugix/rugix/refs/tags/v0.8.0/schemas/rugix-bundle-manifest.schema.json
...
When installing an update bundle, you can use `--bundle-hash <hash>` where `<hash>` is a hash of the bundle's header that can be obtained with:

```shell
rugix-bundler hash <bundle path.rugixb>
```

This is not a hash over the entire bundle but just the bundle's header.
The header, in turn, contains hashes of the payload headers as well as of the payloads themselves, forming a [Merkle tree](https://en.wikipedia.org/wiki/Merkle_tree).
...
Bundle verification is mandatory by default. Either a valid [embedded signature](./signed-updates) or a bundle hash must be provided.
See the [Signed Updates reference](./signed-updates) for details on configuring signature-based verification.
...
The example above specifies a _block encoding_ for each payload.
A block encoding adds a _block index_ for the respective payload to the bundle.
To this end, the payload file is first divided into blocks.
...
Block deduplication requires two assumptions:
...
This is orthogonal to block deduplication.
...
You will find the most recent version of this schema [on GitHub](https://github.com/rugix/rugix/blob/main/schemas/rugix-bundle-manifest.schema.json).
