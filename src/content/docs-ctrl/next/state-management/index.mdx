...
import ConfigSchema from "../../../../schemas/rugix-ctrl-state.schema.json";
...
<p style={{ marginLeft: "2rem" }}>
  **✨ Feature Highlights ✨**<br />✅ Straightforward selective persistence of system state through updates.<br />✅ Protection against accidental state and system partition corruption.<br />✅ Off-the-shelf factory reset functionality.
</p>
...
To persist state, certain files or directories can be explicitly bind mounted from the data partition, thereby ensuring that their content persists through updates.
...
[^container]:
    This is only an analogy to explain the idea.
...
As explained above, state that should persist through updates (and reboots) must be explicitly declared and is then managed by Rugix Ctrl.
...
You will find the most recent version of this schema [on GitHub](https://github.com/rugix/rugix/blob/main/schemas/rugix-ctrl-state.schema.json).
