...
Rugix Ctrl handles both: its core mechanism provides atomic [system updates](./over-the-air-updates) with rollback, while [Rugix Apps](./application-updates/index) extends it with first-class lifecycle management for application workloads, including versioning, crash recovery, and persistent state per application.
...
To manage your devices and push updates to them, Rugix Ctrl is compatible with several [fleet management solutions](./advanced/fleet-management) and can be easily integrated into your application, e.g., by offering your users the ability to upload a firmware update in your own UI.
...
:::tip
Rugix Ctrl can be integrated with [Rugix Bakery](/docs/bakery/) or [Yocto](./advanced/yocto-integration), both of which are fully supported.
