...
By setting the `snapshot` parameter of the `core/debian-bootstrap` recipe (more on parameters [later](/docs/bakery/recipes)), the resulting layer will be based on a particular [Debian snapshot](https://snapshot.debian.org/), thereby leading to a semi-reproducible[^1] build.
...
GitHub](https://github.com/rugix/rugix-bakery/blob/main/schemas/rugix-bakery-layer.schema.json).
