...
Check out the [compatibility documentation](/docs/ctrl/next/updates/compatibility) for the complete component model and practical examples.

## A Clear Privilege Boundary

Rugix Ctrl performs operations that inherently require root privileges: writing partitions, changing bootloader state, managing applications, and resetting persistent state. A local management interface still should not need unrestricted root access just to invoke those operations.

The new [privileged operation daemon](/docs/ctrl/next/reference/privileged-daemon) separates the caller from the privileges required to carry out a request. A root-owned Rugix Ctrl process executes typed operations on behalf of unprivileged clients and enforces its own policy. Sensitive operation families, including factory reset, system commit, explicit reboot, and application lifecycle management, are disabled by default and can be enabled individually.

[Rugix Admin](https://github.com/rugix/rugix-admin), the project's experimental local web interface, now uses this daemon and has moved into its own repository, with its own installer and release artifacts. As an experimental component, Rugix Admin was never part of Rugix Ctrl's compatibility promise. The move lets it evolve independently while keeping the privilege boundary explicit.

## Booting Through Failures

Rugix Ctrl runs early during boot to mount persistent state, prepare the root filesystem, and coordinate with the configured boot flow. Failures at this stage need a deliberate policy because the normal userspace environment is not available yet.
...
Deployments can also configure an in-memory overlay as a fallback when the persistent root overlay cannot be mounted. Boot flows now declare whether they can recover from a userspace failure, allowing Rugix Ctrl to choose an appropriate fallback. Systems without a boot flow are supported as well.
