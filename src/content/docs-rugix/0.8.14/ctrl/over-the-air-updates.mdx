...
To configure Rugix Ctrl for such custom use cases, please have a look at the [System Configuration](/docs/0.8.14/ctrl/advanced/system-configuration) section of this documentation.
...
If you want your own integration, check out the [documentation on boot flows](/docs/0.8.14/ctrl/advanced/boot-flows).
:::
...
- Partition 6: Contains any persistent data and state (see [State Management](/docs/0.8.14/ctrl/state-management)).
...
[^non-a-b]: The following commands assume that you use a traditional A/B update scheme. If you use a non-A/B update scheme, the commands will require you to explicitly specify the [boot group](/docs/0.8.14/ctrl/advanced/system-configuration).
...
You can also _defer_ the reboot with `--reboot deferred` (only for A/B setups, requires [State Management](/docs/0.8.14/ctrl/state-management)).
...
:::tip
Use a [_pre-commit hook_](/docs/0.8.14/ctrl/hooks#system-update-hooks) to ensure that a commit is only possible when the system is in proper working order.
:::
