...
Note that custom slots are incompatible with block deduplication of the update payload (see [Update Bundles](./update-bundles)).
...
For further details on boot flows, we refer to the [Boot Flows](./boot-flows) section.
