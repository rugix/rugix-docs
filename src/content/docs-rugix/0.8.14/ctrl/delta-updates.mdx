...
You can read more about block indices in the [advanced section on Update Bundles](/docs/0.8.14/ctrl/advanced/update-bundles).
...
Rugix's static delta updates require the pre-computation of explicit _patches_ to go from the old version to the new version. This is done with Rugix Bundler's `delta` subcommand, which takes the old and the new update bundle and computes a patch bundle:
