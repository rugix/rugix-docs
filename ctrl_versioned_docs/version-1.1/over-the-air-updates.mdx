...
When installing an update, Rugix Ctrl requires either a valid [embedded signature](./signed-updates.md) or a bundle hash (via `--bundle-hash`).
...
For signature-based verification, Rugix Ctrl supports [embedded CMS signatures](./signed-updates.md) in update bundles.
Root certificates for verification can be configured in `/etc/rugix/ctrl.toml` or provided via the `--root-cert` CLI option.
See [Signed Updates](./signed-updates.md) for details on signing bundles and configuring signature verification.
